...
class Passageway extends Component {
  ...
  addClick() {
    const {
      passagewayStore: { initAdd, getVirtualList },
    } = this.props
  ...
  async alarmOk() {
    const {
      passagewayStore: { a_tableData, alarmDataChange },
    } = this.props
    ...
    const {
      passagewayStore: { alarmBatchSave },
    } = this.props
    const params = {
  ...
  channelTypeChange(value) {
    const {
      passagewayStore: { getChildTable, c_tableParmas },
    } = this.props
    const params = {
      ...c_tableParmas,
      F_ChannelType: value,
  ...
  onExportClick() {
    downExcel("/device_channel/toExcel.do?deviceID=" + this.state.currentDevice)
  }
  editVirtual() {
    const fields = this.state.fields
    ...
    getEditVirtual(params).then((data) => {
      ...
      _.map(relateChannelList, (item, i) => {
        const {
          passagewayStore: { virtualDevList },
        } = this.props
        const obj = {}
        ...
        const currnetDev = _.filter(virtualDevList, (app) => {
          if (currentDevId) {
            ...
            return parseInt(app.deviceID) === parseInt(this.state.currentDevice)
          }
        })
