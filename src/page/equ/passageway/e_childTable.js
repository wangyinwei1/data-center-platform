...
class Regional extends Component {
  ...
  onExpand(expanded, record) {
    const {
      passagewayStore: {c_expandedRowsChange, getGrandsonTable, s_tableParmas},
    } = this.props;
    if (expanded) {
  ...
  render() {
    ...
    const s_tableData = toJS(passagewayStore.s_tableData);
    const tableData = (s_tableData && s_tableData.varList) || [];
    const columns = columnData();
    const nesting = {
      expandedRowRender: this.expandedRowRender,
      onExpand: this.onExpand,
      expandedRowKeys: toJS(passagewayStore.c_expandedRows),
    };
    return (
      <div>
        <Table
          pagination={false}
          loading={passagewayStore.s_loading}
          nesting={nesting}
          columns={columns}
          data={tableData}
        />
