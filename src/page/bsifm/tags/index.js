...
//实例
@inject('regionalStore', 'tagsStore')
@observer
@mixin(cascader)
class Regional extends Component {
  ...
  onCascaderChange(value, selectedOptions) {
    ...
    tagsStore.changeSelectedValue('').then(() => {
      tagsStore.getList(params);
    });
  }
  async _initLoading() {
    const {regionalStore, tagsStore} = this.props;
  ...
  componentDidMount() {
    const {tagsStore, router} = this.props;
  ...
  //渲染
  render() {
    const {tagsStore, regionalStore} = this.props;
