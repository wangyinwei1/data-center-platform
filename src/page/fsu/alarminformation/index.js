...
class Site extends Component {
  ...
  //以下级联方法
  componentDidMount() {
    const {fsu_alarminformationStore: {getTable}} = this.props;
    const params = {
      page: 1,
      keywords: '',
      number: 10,
  ...
  //添加功能
  add() {
    const {fsu_alarminformationStore} = this.props;
    fsu_alarminformationStore.getGoAdd();
    this.setState({
      editShow: true,
      type: 'new',
    });
  ...
  initFromValue(data, mode, item) {
    this.setState(({fields}) => {
      ...
      formValue.alarmMsg.value = data.alarmMsg || '';
      formValue.alarmType.value = data.alarmType || 0;
      formValue.levID.value = data.levID || undefined;
      return {
        fields: {
          ...fields,
          ...formValue,
  ...
  editClick(item) {
    const {fsu_alarminformationStore} = this.props;
    fsu_alarminformationStore.getEidtData({msgID: item.msgID}).then(data => {
  ...
  onDeleteOk() {
    ...
    this.c_onDeleteOk(fsu_alarminformationStore, params);
  }
  onDeleteCancel() {
  ...
  onEditOk() {
    ...
    if (hasError[0]) {
      ...
      const {fsu_alarminformationStore: {save, editSave}} = this.props;
      const params = {};
  ...
  onSearch(value) {
    const {fsu_alarminformationStore} = this.props;
    const params = {
      ...fsu_alarminformationStore.tableParmas,
      keywords: encodeURIComponent(value),
      page: 1,
    };
    fsu_alarminformationStore.search(params);
  }
  //table分页
  onShowSizeChange(current, pageSize) {
    const {fsu_alarminformationStore} = this.props;
    this.c_onShowSizeChange({current, pageSize}, fsu_alarminformationStore);
  }
  onPageChange(pageNumber) {
    const {fsu_alarminformationStore} = this.props;
    this.c_onPageChange({pageNumber}, fsu_alarminformationStore);
  }
  ...
  render() {
    const {fsu_alarminformationStore} = this.props;
    const tableData = toJS(fsu_alarminformationStore.tableData.Data) || [];
    const pagination = toJS(fsu_alarminformationStore.tableData.Data) || {};
          ...
          <div className={styles['min_width']}>
            ...
            <div className={styles['table_wrap']}>
              <Table
                ...
                columns={columns}
                loading={fsu_alarminformationStore.loading}
